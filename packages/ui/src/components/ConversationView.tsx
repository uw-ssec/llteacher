    ...
    <div className="conversation-column">
      ...
      <Composer
        ...
        /* #274 redesign: Stop moved from its own row above the composer into
           ...
           never-unmounted button with Send. Composer.tsx's own comment on that
           button covers why the shared element is what makes #327's focus
           guarantee structural. */
        onStop={
          onStop
            ? () => {
                // #317 review, #345: flags the next isSending->false
                // transition as caused by Stop (see turnCompleteAnnouncement's
                // own effect above). Composer only invokes this in its Stop
                // identity, which requires isStopActionable, so it can't fire
                // from a click that didn't actually stop anything.
