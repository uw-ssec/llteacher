          ...
          {m.parts.map((part, i) => {
            if (part.type === "text") {
              /* Markdown owns its own block structure, so the raw text goes
                 straight to MessageMarkdown. Wrapping it in a <p> here trapped
                 the markdown inside a block element it needed to own: "\n\n"
                 paragraph breaks collapsed to spaces and a leading "# " came
                 out as a literal hash. Message.tsx keeps a shim that unwraps a
                 bare <p>-with-string-child for exactly this shape, but relying
                 on it from the one call site we control would be leaving the
                 bug in and papering over it downstream. */
              return <MessageMarkdown key={`text-${m.id}-${i}`}>{part.text}</MessageMarkdown>;
            }
