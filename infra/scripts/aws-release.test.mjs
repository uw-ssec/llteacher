...
function fixture(t, scenario = {}) {
  ...
  const outputs = { clusterName: 'production-cluster', appSubnetIds: ['subnet-01234567'], appSecurityGroupId: 'sg-01234567', ecrRepositoryUrl: '123456789012.dkr.ecr.us-west-2.amazonaws.com/llteacher-production/app', ...scenario.outputs };
...
if(tool==='pulumi') {
 ...
 out({tasks:[{lastStatus:s.running?'RUNNING':'STOPPED',containers:[{name:'sidecar',exitCode:0},{name:'app',exitCode:s.failFirst&&n===0?1:0}]}],failures:[]});
