...
wait_for_local_service() {
  ...
  for ((attempt = 1; attempt <= attempts; attempt++)); do
    if curl "${curl_args[@]}" --output /dev/null http://127.0.0.1:8080/; then
      return 0
    fi
...
wait_for_local_service
curl "${curl_args[@]}" http://127.0.0.1:8080/
curl "${curl_args[@]}" http://127.0.0.1:8080/admin
curl "${curl_args[@]}" http://127.0.0.1:8080/api/health
