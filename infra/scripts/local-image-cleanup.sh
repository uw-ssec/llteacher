...
docker image ls --filter label=org.llteacher.local=true --format '{{.ID}}' | while read -r image; do
  [[ -z "$image" || "$image" == "$current_image" ]] && continue
  docker ps -a --filter "ancestor=$image" --format '{{.ID}}' | grep -q . && continue
  docker image rm "$image" || true
done
docker builder prune --builder llteacher-local-builder --keep-storage 2GB --force >/dev/null || true
docker system df
