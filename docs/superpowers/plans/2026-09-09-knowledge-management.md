...
- Modify: `apps/web/src/shared/types.ts:433` (the `declare global { interface Env }` block)
- Modify: `apps/web/wrangler.jsonc`

**Interfaces:**
- Consumes: nothing.
- Produces: `ObjectStore` interface, `r2ObjectStore(bucket)`, `memoryObjectStore()`, `materialStorageKey(courseId, materialId, filename)`. Task 14's upload route consumes all of them.

`Env` is hand-declared in `src/shared/types.ts`, not generated — so the binding is added by editing that interface, and `wrangler types` is not part of this workflow.
...
/* --------------------------------------------------------------------------
   ...
   A four-method interface rather than R2Bucket directly, because AWS +
   Pulumi is the settled target platform (#81) and Cloudflare is interim. R2
   is S3-API-compatible, so that migration should swap one implementation
   rather than every caller -- which only holds if the callers never see an
   R2 type. They don't: nothing outside this file imports R2Bucket.

   memoryObjectStore exists so route tests can assert what was stored without
   a network or a miniflare bucket.
   -------------------------------------------------------------------------- */
...
export function r2ObjectStore(bucket: R2Bucket): ObjectStore {
  return {
    ...
    async head(key) {
      ...
      return object
        ? { key, size: object.size, contentType: object.httpMetadata?.contentType ?? null }
        : null;
    },
  };
}
...
```ts
    /* #42: uploaded course materials. R2 today because Cloudflare is the
       interim runtime; the S3 move (#81) swaps the ObjectStore
       implementation in server/storage/objectStore.ts and nothing else,
       because no caller imports an R2 type. */
    MATERIALS: R2Bucket;
```

In `apps/web/wrangler.jsonc`, after `"assets"`:

```jsonc
  "r2_buckets": [
    {
      "binding": "MATERIALS",
      "bucket_name": "llteacher-materials"
    }
  ],
```

Create the bucket once per environment: `npx wrangler r2 bucket create llteacher-materials`.
...
**Interfaces:**
- Consumes: `listMaterialsForCourse` (Task 8), `ObjectStore`/`materialStorageKey`/`r2ObjectStore` (Task 10), `convertToOkf`/`sourceTypeFor`/`MAX_UPLOAD_BYTES`/`ALLOWED_EXTENSIONS`/`extensionOf` (Task 12), `createDocument` (Task 6).
...
import { materialStorageKey, r2ObjectStore } from "../storage/objectStore";
import {
  ALLOWED_EXTENSIONS,
  MAX_UPLOAD_BYTES,
  convertToOkf,
...
export async function uploadMaterialHandler(c: Context<AppEnv>) {
  ...
  try {
    await r2ObjectStore(c.env.MATERIALS).put(key, bytes, {
...
export async function deleteMaterialHandler(c: Context<AppEnv>) {
  ...
  if (removed.storageKey) {
    ...
    try {
      await r2ObjectStore(c.env.MATERIALS).delete(removed.storageKey);
    } catch (error) {
...
export async function reingestMaterialHandler(c: Context<AppEnv>) {
  ...
  const bytes = await r2ObjectStore(c.env.MATERIALS).get(material.storageKey);
  if (!bytes) {
