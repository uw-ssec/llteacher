...
1. **Conversation uniqueness** (data-model doc §3.5 Q1): a partial unique index on `conversations (owner_user_id, section_id) WHERE kind = 'section' AND is_deleted = false` enforces "at most one active section-conversation per user per section." Combined with the existing 1:1 `submissions.conversation_id` unique constraint, this transitively gives "at most one submission per student per section" — the DB-level version of Django's `Submission.clean()` check — without needing to duplicate `owner_user_id`/`section_id` onto `submissions`.
   **Known gap, found during post-epic review (not fixed in this PR):** this only holds within a single conversation's lifetime. `Submission.clean()`'s Django query (`conversation__user=user, conversation__section=section`) checks across *all* conversations ever created for that (user, section), including soft-deleted ones. This schema does not — empirically verified: create conversation A for (user, section), submit it, soft-delete A, create conversation B for the same (user, section) (allowed, since A is no longer active), submit B — the second submission succeeds. A student can accumulate more than one `submissions` row per section by soft-deleting and re-attempting.
   **Deliberately deferred, not a bug left unaddressed**: discussed with the epic owner post-review. Neither Django nor `SERVICES.md` ever defined what "a student resubmits a section" should actually do (allowed at all? new row or replace? does the old conversation get superseded?) — M2 only builds the data layer, and patching the schema ahead of that product decision risks locking in the wrong constraint. Tracked as [#128](https://github.com/uw-ssec/llteacher/issues/128) under M3 (Homeworks & Submissions Parity); revisit `submissions`'/`conversations`' constraints once that issue resolves the open questions.
...
8. **Audit-event append-only enforcement**: deferred to M9, not fixed in M2. For M2, append-only is enforced at the application layer only: `repositories/auditEvents.ts` exports only `recordAuditEvent` (insert), no update/delete function exists anywhere in the codebase for that table. DB-level enforcement (a trigger, not a role-grant revoke — the current single-connection-role setup means a table owner always retains full privileges regardless of REVOKE, so a role-grant approach would need real infra work this repo doesn't have yet) already has an owning issue: [#50](https://github.com/uw-ssec/llteacher/issues/50) (M9, already in epic #54's dependency chain). Commented on #50 with the specific design constraint found during M2 review: a naive "reject every UPDATE" trigger would break `actor_user_id`'s `ON DELETE SET NULL` FERPA-survivorship behavior (which fires as an internal UPDATE and would hit the same trigger) — the trigger needs to specifically allow "only `actor_user_id` changed, set to NULL" while rejecting everything else.
...
12. **`middleware/roles.ts` closed out post-epic** (found + fixed during post-epic review, not part of the original Phase 4 scope): `rolesMiddleware` was the one place still calling `db.query.courseMemberships.findMany()` directly, left as a known caveat when Phase 4 closed since it's pre-existing M1 code, not something #16 asked to touch. Unlike decisions #1/#8 (which needed a product/design decision before fixing), this had no such blocker — the correct fix was already well understood, just deferred. Added `repositories/users.ts` (`listMembershipsForUser`, intentionally taking no `OrgScope`/`CourseScope` since this query is how the middleware discovers a user's scopes in the first place — it can't be scoped to a tenant it hasn't resolved yet) and refactored `roles.ts` to call it. Repo-wide grep now confirms zero direct Drizzle query calls remain under `server/routes/` or `server/middleware/`.
13. **Seed script section count and message variety closed out post-epic** (found + fixed during post-epic review): "Data Analysis with Python" gets a 3rd section ("Summarizing Data with Pandas") to match "2 homeworks with 3–4 sections each" — Django's actual reference only had 2, so this is a deliberate small deviation from exact Django parity in favor of matching the epic's own checklist wording, not a bug. Seeded messages now cycle through 3 patterns (plain text, code embedded in a text part, a tool-call/tool-result pair) instead of one uniform exchange, demonstrating the `parts` jsonb variety that replaced Django's `student/ai/code/code_execution` split (decision #2) rather than trying to reintroduce that enum.
14. **`grades.grader_membership_id` changed from `ON DELETE SET NULL` to `ON DELETE RESTRICT`** (found + fixed in PR #127 review, tracked as [#129](https://github.com/uw-ssec/llteacher/issues/129)): Postgres executes `SET NULL` as a real `UPDATE` on the referencing row, which `grades_grader_consistency_chk` (decision #6) then rejects for any human-graded grade — the two constraints as originally written were mutually incompatible, and deleting a grader's `course_memberships` row would abort with error 23514 the moment any human-graded grade existed. `RESTRICT` makes that state unreachable instead of making the delete crash trying to reach it: a grader's membership can't be deleted while they have recorded grades. A future retention story (reassignment, anonymization) can revisit this deliberately; migration `0006_grades_grader_membership_restrict.sql`.
15. **`scripts/seed.ts`'s `reset()` rescoped to the seed org's cascade subtree, plus a non-local `DATABASE_URL` guard** (found + fixed in PR #127 review, tracked as [#130](https://github.com/uw-ssec/llteacher/issues/130)): `reset()` previously issued unscoped `DELETE`s across 13 tables — every organization's data, not just the seed org's — with no environment guard, and `seed.test.ts` ran `--reset` in `beforeAll`, so plain `npm test` with `DATABASE_URL` set could wipe a shared database (`.dev.vars.example` templates a Neon host). Fixed by deleting only the `organizations` row where `slug = 'seed-org'` and letting the existing FK cascade chain (`courses` → `homeworks` → `sections` → everything org-anchored) do the rest, plus the `isPending = true` scoped `users` delete (users aren't in the org's cascade tree by design). This also fully de-races the suite: the earlier unscoped reset could delete rows other parallel test files were concurrently asserting on; scoping to `seed-org`'s slug means it can never touch another test file's randomly-slugged fixture org. `--reset` against a non-`localhost`/`127.0.0.1`/`*.local` `DATABASE_URL` now refuses unless `--force` is also passed. Verified: full suite (190 tests) passed cleanly across 5 consecutive runs against a local Postgres after the fix.
16. **Seed script now encrypts PII under the app's actual active key id** (found + fixed in PR #127 review, tracked as [#131](https://github.com/uw-ssec/llteacher/issues/131)): `loadCipher()` previously hard-coded `encryptionKeyId: "seed"`, but `IdentityCipher.decryptString` hard-fails on any key id other than `secrets-loader.ts`'s `ACTIVE_KEY_ID` (`"k1"`) — every seeded user's email/display name would throw on decrypt in any real app code path (roster views, profile page), defeating the stated purpose of seeding realistic PII. Fixed by exporting `ACTIVE_KEY_ID` from `secrets-loader.ts` and routing `loadCipher()` through the same `loadIdentityCipherKeys()` the Worker uses, instead of hand-rolling the key import a second time — this also means a future key rotation can't accidentally re-split the two call sites. `seed.test.ts` gained a regression test that decrypts a seeded user's email via the loader-built cipher.
17. **Repository write functions now verify the parent-row chain before writing denormalized tenancy columns** (found + fixed in PR #127 review, tracked as [#132](https://github.com/uw-ssec/llteacher/issues/132)): `createSubmission`, `recordGrade`, and `createConversation` previously trusted the caller's scope tag without checking that a caller-supplied UUID (conversation id, submission id, section id, owner user id) actually belonged to that scope — no DB constraint ties the denormalized `organization_id`/`course_id` columns to the real parent chain. Not exploitable yet (no M3 route calls these functions), but the first route that passes a request-body UUID would have turned each into an IDOR, and a mis-tagged row would then surface through every correctly-filtered read for the wrong org. Fixed following the pattern `appendMessage` already used: `createSubmission` verifies the conversation's course belongs to the org scope; `recordGrade` verifies the submission belongs to the org scope and that any `graderMembershipId` is a membership of the submission's course; `createConversation` verifies `ownerUserId` is a member of the scoped course and (when set) `sectionId` belongs to it. All three throw a plain `Error` on mismatch, matching `appendMessage`'s existing convention (tightening error typing is tracked separately in #136). Negative tests added to `conversations.test.ts` and `submissions.test.ts` for every new check.
18. **`grades.submission_id` and both of `llm_call_logs`' FKs changed from `ON DELETE CASCADE` to `ON DELETE RESTRICT`** (found + fixed in PR #127 review, tracked as [#133](https://github.com/uw-ssec/llteacher/issues/133)): the cascade chain `users → conversations → submissions → grades` (and `→ messages → llm_call_logs`) meant deleting one student user silently erased their grade records (the FERPA education record) and shrank the org's LLM cost/telemetry accounting, with no gate. Fixed with the narrowest possible restrict: `grades.submission_id` (blocks deleting a submission — and so its conversation, and so its owning user — while a grade, AI- or human-graded, still references it; a user with only ungraded submissions can still be deleted normally) and both `llm_call_logs.message_id` and `.conversation_id` (both needed — `conversation_id` is the FK Postgres actually walks first when a conversation cascade-deletes, so restricting only `message_id` would have been a no-op: by the time that check ran, `conversation_id`'s cascade would already have removed the row). This intentionally does not build the full FERPA hard-delete flow (§3.5 Q5, still not started) — it just stops the default cascade from being the accidental policy, matching #129's precedent on `grades.grader_membership_id`. Org deletion is unchanged and still cascades everything including `audit_events`; documented as acceptable only because no code path deletes organizations today (see `docs/architecture/multi-tenant-data-model.md` §3.5 Q5 addendum) — revisit before any real org-deletion feature ships. Migration `0007_gate_user_deletion_cascade.sql`. This change broke three pre-existing tests' assumptions that grades/llm_call_logs cascade-delete freely, and three `afterAll` hooks that relied on that same assumption to clean up organizations — all four updated (two tests rewritten to assert the new restrict behavior instead, one split into a still-cascades-when-ungraded case plus a new blocked case, three `afterAll` hooks now explicitly clear `grades`/`llm_call_logs` before deleting their test orgs). Regression tests added: blocks deleting a conversation with a graded submission, blocks deleting a user with a graded submission, blocks deleting a message/conversation with a logged LLM call.
19. **Row ownership and content-encryption boundary decisions recorded, not implemented** (found + documented in PR #127 review, tracked as [#134](https://github.com/uw-ssec/llteacher/issues/134)): two implicit boundaries were written down rather than coded, since neither had a route/consumer today to implement against. (a) Row ownership within a scope — `softDeleteConversation`/`appendMessage` enforce `CourseScope` only, so any course member can currently act on any other student's conversation by UUID — is now recorded in `apps/web/ARCHITECTURE.md`'s "Row Ownership (Within a Scope)" section as the M3 route layer's job, extending the doc's existing "guards decide who; repositories decide which org's rows" split with a third tier; both repository functions got a cross-linking comment. (b) The content-encryption boundary (`messages.parts`, `grades.feedback`/`rubric`, `student_profiles.summary`/`mastery_signals`) is now recorded in `docs/architecture/multi-tenant-data-model.md` decision 4a, alongside decision 4's identity-PII encryption: `messages`/`grades` stay plaintext (volume + queryability + Neon at-rest disk encryption already covers the exfiltration threat model), but `student_profiles` should not — §3.2 already documents accommodations as in-scope for that table, a categorically more sensitive FERPA/ADA data class than chat or grade content — filed as a follow-up, [#137](https://github.com/uw-ssec/llteacher/issues/137) (M9), since encrypting it is a real migration (and `IdentityCipher` only handles strings today, not `student_profiles.mastery_signals`'s `jsonb`), not a docs-only fix.
20. **Scope minting hardened; five missing indexes added** (found + fixed in PR #127 review, tracked as [#135](https://github.com/uw-ssec/llteacher/issues/135)): `orgScope()`/`courseScope()` renamed to `unsafeOrgScope()`/`unsafeCourseScope()` (all ~20 test call sites across `scope.test.ts`, `homeworks.test.ts`, `submissions.test.ts`, `conversations.test.ts` mechanically renamed, behavior unchanged) to signal "zero validation, don't reach for this in route code" the way the plain names didn't. Added `courseScopeFromAuthContext(authContext, courseId): CourseScope | null`, the first scope constructor that can actually fail closed — it only mints a scope when `authContext.isMemberOf(courseId)` is true. `routes/homeworks.ts`'s two mint sites (`listHomeworksHandler`, `createHomeworkHandler`) migrated to it. No equivalent `orgScopeFromAuthContext` yet — `AuthContext.memberships` is `CourseMembership` rows with no `organization_id` column, and no production route mints an `OrgScope` from request context today to justify the join that would need. This surfaced a real gap in `routes/homeworks.test.ts`'s `fakeAuthContext` test double: it hand-set `isInstructorOf` per test without deriving `isMemberOf` from the same `memberships` array the way production `rolesMiddleware` does, so the "allows the instructor... to create a homework" test broke against the new check — fixed by making the double derive `hasRole`/`isMemberOf`/`isInstructorOf` from `memberships` by default (matching `roles.ts`), same as production, while still letting individual tests override any one predicate. Five indexes added: `citations (message_id)`, `citations (grade_id)` (render-time lookup + cascade path), `conversations (section_id)` (the existing partial unique index leads with `owner_user_id`, unusable for "all conversations on this section"), `grades (grader_membership_id)` and `llm_call_logs (llm_config_id)` (both RESTRICT/SET NULL cascade paths). Migration `0008_scope_helper_and_indexes.sql`.
21. **Minor cleanups batch** (found + fixed in PR #127 review, tracked as [#136](https://github.com/uw-ssec/llteacher/issues/136)): (a) `conversations_owner_section_active_uq`'s comment reworded — it no longer claims to transitively cap submissions per (user, section), which #128 already disproved; the schema comment just hadn't caught up to the docs. (b) `conversations.updatedAt` gained `.$onUpdate(() => new Date())` — the only `updatedAt` column in `runtime.ts`, and `softDeleteConversation` was already leaving it stale. (c) `llm_call_logs.message_id` changed from `notNull()` to nullable — an error-flagged call that fails before the assistant message persists has nothing to attach to; `unique()` still holds (Postgres doesn't treat multiple `NULL`s as duplicates). (d) `citations` gained a `citations_span_range_chk` CHECK: both `span_start`/`span_end` null, or both set and sane (non-negative, start ≤ end) — a half-set or backwards span is now rejected at insert time instead of silently stored. (e) `appendMessage` now filters `isDeleted = false` (previously allowed appending to a soft-deleted conversation) — its error-typing (plain `Error` → generic 503, not the more honest 404) and non-transactional check-then-insert are left as-is, explicitly deferred to land together with the ownership work when #134 does. (f) `repositories/users.ts`'s `listMembershipsForUser` doc comment now notes it doesn't filter `droppedAt` (pre-existing M1 behavior, now flagged so a future consumer doesn't inherit it silently). (g) `seed.ts`'s submission-rate coin flip changed from `Math.random() < 0.6` to the Django reference's deterministic `(i % 5) < 3` shape — reproducible seed output instead of drifting between 4-vs-5 submissions per run. (h) CI workflow gained `timeout-minutes: 15`, and `ENCRYPTION_KEY`/`BLIND_INDEX_KEY` switched from the two `CI_SEED_*` repo secrets (never added — this had been the one remaining CI blocker from decision 11's action item) to `openssl rand -base64 32`-generated ephemeral values, since nothing needs them to survive past the job — this closes the CI blocker outright rather than just documenting it, and removes the fork-PR-secrets footgun as a side effect. (i) Test hygiene: `submissions.test.ts`'s `recordGrade` test title no longer claims a rejection assertion its body never made (the real CHECK-constraint test already lives in `runtime.test.ts`); `conversations.test.ts`'s `afterAll` now deletes the orgs (which cascades courses) and the two fixture users instead of only deleting courses and leaving orgs/users behind; two new tests exercise `appendMessage`'s previously-untested paths (wrong-scope rejection, soft-deleted-conversation rejection, the latter also covering item (e)). Migration `0009_minor_cleanups_from_review.sql`.

## Round-2 review (PR #127 @ commits 67da496/9d94877) — decisions 22-24

22. **Org deletion is actually blocked, not "unchanged," whenever a grade or LLM log exists; `seed.ts reset()` made restrict-proof** (found + fixed in round-2 review, tracked as [#138](https://github.com/uw-ssec/llteacher/issues/138)) — ~~decision 18's own addendum claimed org deletion "is unchanged and still cascades everything" — false after migration 0007.~~ **This entire decision was itself wrong and is superseded by decision 23 below** — the round-2 reviewer's premise (that the RESTRICT walls block org deletion) doesn't hold; see decision 23 for the corrected mechanism and fix. `scripts/seed.ts reset()`'s pre-clearing of the seed org's `grades`/`llm_call_logs` (described below) was kept as harmless defense-in-depth, not reverted, even though it turned out not to be load-bearing.

23. **Correcting decision 22: org deletion actually cascades grades/llm_call_logs away, it does not block** (found in the M2 full audit, [#138](https://github.com/uw-ssec/llteacher/issues/138) reopened): decision 22 asserted deleting an org walks `courses → conversations → submissions` first and hits the RESTRICT walls before its own direct cascades ever run. Verified empirically (minimal FK repro + the full 12-migration schema) that this is backwards: `organizations` has its own *direct* cascade FKs — `grades.organization_id`/`llm_call_logs.organization_id` — created *earlier* (migrations 0004/0005) than the deeper `courses → conversations → submissions` chain that eventually reaches `grades.submission_id`'s RESTRICT. Postgres fires a row's FK triggers in constraint-creation order, so those direct cascades fire and delete the org's `grades`/`llm_call_logs` first — there's nothing left for the deeper RESTRICT to block by the time that path is reached. **`DELETE FROM organizations` succeeds and silently cascades away its grades, LLM call logs, and audit events; there is no gate.** The RESTRICT still genuinely protects user/membership/conversation/submission-level deletes (control-tested: deleting a conversation, which reaches `grades` only through the submission RESTRICT, is correctly blocked) — it just never applies to org-level deletes. Corrected in `docs/architecture/multi-tenant-data-model.md` §3.5 Q5 and the `grades` schema comment in `runtime.ts` to state the true (cascade, not block) behavior; explicitly notes that org-level retention, if ever required, needs a real design change (e.g. `RESTRICT` on the direct org-level FKs, or a soft-delete on `organizations`) that doesn't exist today. `scripts/seed.ts reset()` keeps pre-clearing the seed org's `grades`/`llm_call_logs` before deleting the org row (harmless defense-in-depth, self-documenting delete order) but this was never load-bearing for `--reset` to succeed — the org delete would cascade them away either way. `seed.test.ts`'s regression test was tautological (passed whether or not `reset()` pre-cleared, since the org delete itself always succeeds) and was replaced with one asserting the actual documented behavior.
24. **`droppedAt` filtered across the entire membership → scope-minting → repository-check chain** (found + fixed in round-2 review, tracked as [#139](https://github.com/uw-ssec/llteacher/issues/139)): decision 21(f) recorded the missing filter as a comment-level caveat because nothing wrote `droppedAt` yet and nothing depended on `listMembershipsForUser` for anything but display. Decision 20's `courseScopeFromAuthContext` (#135) changed that: it's now the sanctioned production path from a dropped membership all the way to a minted `CourseScope`, so the caveat became a real access-control gap the moment that helper shipped. `listMembershipsForUser` now filters `droppedAt IS NULL` (doc comment upgraded from "caveat" to "enforced"); `createConversation`'s owner-membership check and `recordGrade`'s grader-membership check both gained the same filter. Swept every other `courseMemberships` query site: `UserIdentityService.ts`'s account-merge/reconciliation queries and `ProfileService.ts`'s profile-stats query were deliberately left unfiltered — the former needs to see dropped rows to correctly dedupe/merge without violating the membership unique constraint, and the latter is a display query outside the security concern this issue is about (whether a dropped course should still count toward profile stats is a separate product question, not an access-control bug). Real-DB regression tests: a dropped membership is excluded from `listMembershipsForUser` while an active one in the same query isn't, and — mirroring `rolesMiddleware`'s own derivation logic against the real query result, since `makeDb()`'s Neon HTTP driver can't run a full HTTP-level route test against local Postgres (decision 9) — a dropped membership no longer satisfies `isMemberOf`/`isInstructorOf`; `createConversation` rejects a dropped owner even though the membership row still exists.
25. **Round-2 minors batch** (found + fixed in round-2 review, tracked as [#140](https://github.com/uw-ssec/llteacher/issues/140)): (a) the `--reset` hostname guard checked `new URL(...).hostname` alone, but node-postgres's connection-string parser gives a `host`/`hostaddr` query param precedence over the URL's own authority hostname — `postgres://localhost/db?host=ep-prod.neon.tech` passed the guard while actually connecting remote; fixed by checking those query params first, and `.local` was dropped from the local-allowlist (ambiguous whether it means "my machine" or "a corporate AD host," and nothing in this repo's dev/CI setup relies on it). `--force` now prints the resolved target hostname before proceeding. (b) `createSubmission` now requires `isDeleted = false` and `kind = 'section'` on the target conversation — a submission against a deleted or free-standing tutor conversation was representable before. (c) `recordGrade`'s grader-membership check gained a role predicate (`instructor`/`admin`, matching `AuthContext.isInstructorOf`'s own role set) alongside #139's `droppedAt` filter — a student membership no longer passes as a grader; who may *call* `recordGrade` for a course stays the route layer's job per `ARCHITECTURE.md`'s documented split, this only checks that the cited grader membership is capable of grading. (d) Added the missing negative test: `recordGrade` with a cross-org `submissionId` (the scope check itself was previously unexercised; only the grader check had a test). (e) `citations_span_range_chk` gained an explicit `(span_start IS NULL) = (span_end IS NULL)` equality clause — Postgres CHECK constraints treat a NULL expression result as passing, not failing, so `(5, NULL)` silently passed the old range-only clause (`span_end >= 0` evaluates to NULL, not FALSE). (f) Filed [#141](https://github.com/uw-ssec/llteacher/issues/141) as the confirmed tracking issue for typed not-found errors across all four repository mismatch throws (`createSubmission`, `recordGrade`, `createConversation`, `appendMessage`) and their 404-vs-503 route-layer mapping, cross-linked to the three routes (#5, #22, #75) that will eventually wire each one up; all four functions' comments updated to point at it. (g) `reset()`'s user deletion rescoped from a global `isPending = true` proxy to the seed users' own computed email blind indexes (`isPending = true` kept as a redundant second guard, not removed) — a real invite/roster-provisioning flow creating its own pending users would otherwise get wiped by `--reset --force` on a shared DB. (h) Cosmetics: introduced a `SeedUsageError` class so the top-level `catch` prints just the message for expected/user-actionable failures (missing env var, unsafe host, already-seeded) instead of a full stack trace, reserving the stack trace for genuinely unexpected errors; noted in a comment that `appendMessage` never bumps `conversations.updatedAt` (fine today, nothing reads it for "recently active" ordering yet). Migration `0010_round2_span_check_fix.sql`.
