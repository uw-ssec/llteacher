...
describe.skipIf(!DATABASE_URL)("submissions repository", () => {
  ...
  beforeAll(async () => {
    ...
    async function seed(label: string) {
      ...
      const [conv] = await db
        .insert(conversations)
        .values({ ownerUserId: user.id, courseId: course.id, sectionId: null, kind: "tutor", title: "t" })
        .returning({ id: conversations.id });
      return { orgId: org.id, conversationId: conv.id };
    }
  ...
  it("createSubmission writes under the given org scope", async () => {
    const created = await createSubmission(db, orgScope(orgAId), conversationAId);
  ...
  it("getSubmissionByConversation scoped to org B returns nothing for an org-A conversation", async () => {
    await createSubmission(db, orgScope(orgAId), conversationAId);
    const result = await getSubmissionByConversation(db, orgScope(orgBId), conversationAId);
    expect(result).toBeUndefined();
  });

  it("cross-org isolation: creating submissions under both orgs, an org-A-scoped list never includes org-B rows", async () => {
    await createSubmission(db, orgScope(orgBId), conversationBId);
    const found = await getSubmissionByConversation(db, orgScope(orgAId), conversationBId);
  ...
  it("recordGrade writes an AI grade with no grader and a human grade rejects graded_by_ai=true with a grader set", async () => {
    const sub = await createSubmission(db, orgScope(orgBId), conversationBId);
