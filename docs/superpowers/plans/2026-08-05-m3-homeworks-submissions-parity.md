...
15. **Phase 7's own final-review fix wave found the same bug class a third time, in a different field — `releasedAt: ""` broke the very feature Phase 7 built.** Task 24 added an unpublish-with-existing-activity 409+confirm flow; a final integration review of Phase 7 found it was completely unreachable in production, because `HomeworkForm`'s uncontrolled `releasedAt` `<input type="datetime-local">` sends `""` for an untouched field (never `undefined`) — exactly Decision 14/C1's pattern, this time hitting `publishHomeworkHandler`'s own pre-existing `releasedAt` validation, which ran *before* Task 24's new check could ever fire, 400ing every real unpublish attempt with "releasedAt must be a valid date." A second, related bug in the same validation: it also rejected unpublish requests that re-sent an already-scheduled homework's own unchanged *past* `releasedAt` as "must be in the future," even though `releasedAt` is irrelevant to an unpublish (`updateHomeworkPublishState` ignores it whenever `publish` is `false`). Fixed by only validating `releasedAt` when `body.publish && body.releasedAt` — unpublish requests never validate it at all now, and an empty string is treated as absent for a publish request too. Same review also found and fixed: `listHomeworksForCourse` had no `orderBy`, so Task 26's `HW-00N` record badges (sourced from array position) renumbered on every edit — fixed with `orderBy: (h, {asc}) => [asc(h.createdAt)]`, an immutable sort key so badges stay attached to the same homework permanently; `SubmissionsDataLoader` in `App.tsx` was the one fetch left in the admin app with no `res.ok` guard (pre-existing since Task 21, closed here since Task 26 had just established the guard pattern one component away); and `GET .../homeworks` was only half role-aware (Task 23 derived status but never filtered) — any course member, not just instructors, could see draft/scheduled titles and descriptions, inconsistent with the student route's own deliberate visibility policy — fixed by filtering to non-draft/non-scheduled for non-instructors, mirroring `studentHomeworks.ts`'s exact predicate. One consolidated fix dispatch plus one scoped re-review, both clean, closed Phase 7 out — see the SDD ledger for the full findings.
...
16. **External code review (PR #154, @cdcore09 at `78978d1`) found 8 issues the whole-branch review and Phase 7's own final review both missed, because both reviewed against this plan's tasks/decisions, never against a fresh adversarial read of the merged diff.** Filed as #155-#162; #157 was closed by the reviewer themselves as superseded by #163 (deleting the neon-http driver split entirely once the platform moves to RDS/node-postgres, M12 — building batch-path test coverage for code scheduled for deletion wasn't worth it). The other 7 were implemented directly (no SDD dispatch — well-specified, single-file-scoped fixes each with exact requirements and implementation notes already in the issue text):
    - **#155/#156 (blocking, landed together per the review's own request)**: `GET .../homeworks/:homeworkId` returned every section's model solution, and served draft/scheduled homeworks, to any enrolled course member including students — the route's `isInstructorOf` check existed (used only for `editableBy`) but was never applied to `solution` or to a status gate. This is the same class of gap Decision 15's list-route fix closed for `GET .../homeworks` — the detail route was the one sibling read path that never got the equivalent treatment, and nothing in Phase 7's audit compared the two routes' visibility rules against each other. Fixed by hoisting the check and gating both.
    - **#158**: `getStudentHomeworksForUser` had the exact N+1 shape Decision 10's `getHomeworkSubmissionsMatrix` was built to avoid (per-section queries in a per-homework loop) — the two functions solve structurally identical problems but were written in different tasks (Phase 2 vs. Phase 5) and nobody cross-checked the earlier one against the later one's pattern. Fixed by applying the same inArray-then-join-in-memory template, verified with a query-count test.
    - **#159**: `submissionRate` conflated "any engagement" with "submitted" — latent (nothing renders it today), but wrong data in the API contract. Given its own `submittedStudents` count rather than reusing `activeStudents`.
    - **#160**: `useStudentHomework` was the one client fetch in either app that never checked `res.ok` — the admin data loaders all gained this pattern during Decision 14's fix wave, but the student app's own hook was never audited against it. Fixed, plus two related hardcoded-default bugs in the same function (`currentSection`, `hwNumber`) folded in per the issue's own note.
    - **#161**: `PATCH .../homeworks/:homeworkId` validated `llmConfigId`'s shape (Decision 14's C1 fix) but never its org ownership — a real cross-tenant credential problem once M4 resolves LLM credentials through this column. Fixed with a scope check against the specific course's org (deliberately not `getOrgScopesForUser`'s full list — see the code comment on `getOrgScopeForCourse`).
    - **#162**: three cleanups (an unfilled TODO now links to #166, an implementer-directed comment rewritten into a real explanation, a sort comparator that returned `1` for both orderings of a tie).
