...
1. **Conversation uniqueness** — **resolved.** The active case was closed in M2 by a partial unique index enforcing at most one active section-conversation per (user, section); see the plan doc's decision #1. The remaining half — that this did *not* replicate `Submission.clean()`'s guarantee across a soft-delete-and-recreate cycle, so a student could accumulate more than one `submissions` row for the same section — is closed by [#128](https://github.com/uw-ssec/llteacher/issues/128). `submissions` now carries denormalized `user_id`/`section_id`, tied to the owning conversation by a composite foreign key and capped by `UNIQUE (user_id, section_id)`; the pair could not previously be constrained at all, because `submissions` named neither the student nor the section. The product question M2 deferred ("can a student resubmit, and what happens to the old conversation") was answered as **restart voids the submission**: the student returns to a not-submitted state and re-submits the new conversation, and a graded submission cannot be restarted at all. Rationale and rejected alternatives: [docs/superpowers/specs/2026-08-11-submission-uniqueness-design.md](../superpowers/specs/2026-08-11-submission-uniqueness-design.md).
...
5. **Hard-delete path** — not started; M2's `conversations`/`messages`/`submissions` still cascade-delete on their parent. **Post-epic addendum (issue #133):** the one exception is `grades` and `llm_call_logs`, which are `ON DELETE RESTRICT` from their immediate parent (`grades.submission_id`; `llm_call_logs.message_id` and `.conversation_id`) — until a real FERPA hard-delete flow exists, deleting a *user* (or a conversation/submission upstream of it) is blocked outright if it would silently destroy a grade record or LLM cost/telemetry log, rather than letting the cascade erase them as a side effect. A user with no graded submissions and no logged LLM calls can still be deleted normally.

   **Correction (issue #341, M4):** the `llm_call_logs` half of the above stopped being true the moment `llm_call_logs` gained a real writer (M4's #317, one row per chat turn) — RESTRICT then permanently blocked `deleteHomework`/`updateHomework`'s section-removal path for any section with even one logged chat turn, not just graded ones. Migration 0036 flips `llm_call_logs.message_id`/`.conversation_id` to `ON DELETE SET NULL`, matching `llm_config_id`'s existing pattern on the same table: the cost/telemetry row survives the delete, detached, rather than blocking it or being silently destroyed. `grades.submission_id` is unaffected and keeps RESTRICT — the reasoning below (and the org-deletion caveat) still applies to `grades` alone. **This gate does not apply to organization deletion (issue #138, correcting a wrong claim made twice in earlier revisions of this paragraph):** `organizations` has its own *direct* cascade FKs — `grades.organization_id` and `llm_call_logs.organization_id` — created earlier (migrations 0004/0005) than the deeper `courses → conversations → submissions` path that eventually reaches `grades.submission_id`'s RESTRICT. Postgres fires a row's FK triggers in constraint-creation order, so `DELETE FROM organizations` fires the org's direct cascades to `grades`/`llm_call_logs` first, deleting them, *before* the submission-level RESTRICT is ever reached — there's nothing left for it to block by the time that path runs. Verified empirically (minimal FK repro and the full 12-migration schema): **deleting an organization succeeds and silently cascades away its grades, LLM call logs, and audit events, with no error and no gate.** The RESTRICT genuinely protects user/membership/conversation/submission-level deletes (verified as a control: deleting a conversation, which reaches `grades` only through the submission RESTRICT, is correctly blocked) — it just doesn't reach org-level deletes, because the org's own direct cascade wins the race. If org-level grade/log retention is ever actually required, that needs a real design change (e.g. `RESTRICT` on the direct `grades.organization_id`/`llm_call_logs.organization_id` FKs, or a soft-delete on `organizations`) — not implemented, tracked separately if/when a real org-offboarding flow is built. `scripts/seed.ts`'s `reset()` still pre-clears the seed org's `grades`/`llm_call_logs` before deleting the org row; that's harmless defense-in-depth (and keeps the delete order self-documenting) but is not load-bearing for `--reset` to succeed.
...
1. **Conversation uniqueness.** ~~Should `(user, section)` have at most one *active* (non-deleted) conversation? Current implicit answer is "yes, soft-delete enforces it"; make it explicit with a partial unique index.~~ **Resolved** — partial unique index in M2 for the active-conversation half, and [#128](https://github.com/uw-ssec/llteacher/issues/128) for the submissions half it left open. See the implementation-status note above.
