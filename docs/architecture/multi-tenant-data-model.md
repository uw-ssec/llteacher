...
5. **Hard-delete path** — not started; M2's `conversations`/`messages`/`submissions` still cascade-delete on their parent. **Post-epic addendum (issue #133):** the one exception is `grades` and `llm_call_logs`, which are `ON DELETE RESTRICT` from their immediate parent (`grades.submission_id`; `llm_call_logs.message_id` and `.conversation_id`) — until a real FERPA hard-delete flow exists, deleting a user (or anything upstream of it) is blocked outright if it would silently destroy a grade record or LLM cost/telemetry log, rather than letting the cascade erase them as a side effect. A user with no graded submissions and no logged LLM calls can still be deleted normally. **Correction (issue #138):** the previous version of this addendum claimed org deletion was unaffected and "still cascades everything." That's false. `organizations` also has *direct* cascade FKs from `grades.organization_id` and `llm_call_logs.organization_id`, but those were created later (migrations 0004/0005) than `courses.organization_id`'s cascade (migration 0001) — Postgres fires a row's FK triggers in constraint-creation order, so `DELETE FROM organizations` walks `courses → homeworks/conversations → submissions` first, hits `grades.submission_id` RESTRICT (and, via `conversations`, `llm_call_logs.conversation_id` RESTRICT) on the way, and aborts with a raw `23503` — before the org's own direct cascades to those two tables ever get a chance to run. In practice: **deleting an organization is blocked whenever any grade or LLM call log exists anywhere under it**, the exact same gate user deletion has, just reached by a different path. `audit_events` has no such gate (`audit_events.organization_id` is a plain cascade with no RESTRICT anywhere upstream of it), so it deletes cleanly *whenever the org delete itself isn't blocked* — meaning "org deletion cascades `audit_events`" is only true in the already-narrow case where the org has zero grades and zero LLM logs. **Manual deletion order for the day a real org-offboarding flow needs it**: export/archive `grades` and `llm_call_logs` for the org, delete those two tables' rows for the org (or reassign/anonymize per whatever retention policy exists by then), *then* delete the organization row — at that point the remaining cascade (courses, conversations, `audit_events`, etc.) proceeds cleanly. This is exactly the sequencing `scripts/seed.ts`'s `reset()` now follows for the disposable `seed-org`, so a developer who has chatted against or graded seeded data doesn't get a confusing FK error on `--reset`.
